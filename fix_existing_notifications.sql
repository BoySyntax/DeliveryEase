-- Fix existing notifications and ensure triggers work properly
...
-- Now fix existing notifications for approved orders
-- Find orders that are approved but don't have verification notifications
...
LEFT JOIN existing_notifications en ON ao.order_id = en.order_id
WHERE en.order_id IS NULL;

-- Show the results
SELECT 
    'Fixed notifications for approved orders' as message,
    COUNT(*) as count
FROM (
    SELECT 
        o.id as order_id,
