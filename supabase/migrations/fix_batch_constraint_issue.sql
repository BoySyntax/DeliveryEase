...
SELECT 
    'Pending batches per barangay:' as status,
    barangay,
...
-- 2. Remove the problematic unique constraint
DROP INDEX IF EXISTS unique_pending_batch_per_barangay;

-- 3. Create a more flexible constraint that allows multiple batches when needed
...
-- 4. Update the batch assignment function to handle multiple batches properly
...
-- 5. Create a function to manually assign batches for existing approved orders
...
-- 6. Run the function to assign batches for existing approved orders
SELECT '=== ASSIGNING BATCHES ===' as info;
...
SELECT 
    'Total approved orders:' as status,
    COUNT(*) as count
FROM orders 
WHERE approval_status = 'approved';

SELECT 
    'Total batches:' as status,
    COUNT(*) as count
FROM order_batches;

-- 8. Show final batch distribution
SELECT 
...
FROM order_batches
ORDER BY barangay, created_at DESC; 