-- Migration: Fix same barangay orders being assigned to different batches
-- This ensures orders from the same barangay are always grouped together

-- Update the batch assignment function to prioritize same barangay batches
...
-- Run the fix function to consolidate existing orders
...
COMMENT ON FUNCTION batch_approved_orders() IS 'Updated to prioritize same barangay batching - orders from same barangay will always be grouped together';
COMMENT ON FUNCTION fix_same_barangay_batches() IS 'Fixes existing orders that are incorrectly split across multiple batches for the same barangay';
