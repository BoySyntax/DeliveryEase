-- PERMANENT FIX: Ensure orders go to existing batches until 3500kg limit
-- This migration fixes the core batch assignment logic

-- Step 1: First, consolidate existing duplicate batches
DO $$
DECLARE
...
BEGIN
    RAISE NOTICE 'Consolidating existing duplicate batches...';
...
-- Step 2: Fix the batch assignment function with CORRECT logic
...
BEGIN
    ...
    IF NEW.approval_status = 'approved' AND OLD.approval_status != 'approved' THEN
        ...
        -- DECISION LOGIC: Always try to use existing batch first
        IF existing_batch_id IS NOT NULL THEN
            ...
            IF existing_batch_weight + NEW.total_weight <= 3500 THEN
                -- ✅ ADD TO EXISTING BATCH (this is what you want!)
                UPDATE order_batches 
            ...
            ELSE
                -- Existing batch would exceed 3500kg, create new batch
                RAISE NOTICE 'Existing batch would exceed 3500kg, creating new batch for %', 
                    order_barangay;
                ...
                NEW.batch_id := existing_batch_id;
                RAISE NOTICE 'Created new batch for % with weight %kg', 
...
-- Step 4: Add a simple constraint to prevent empty barangay names
...
BEGIN
            RAISE NOTICE 'PERMANENT BATCH LOGIC FIX COMPLETE!';
        RAISE NOTICE '========================================';
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'NEW BATCH LOGIC ENFORCED:';
    RAISE NOTICE '   • Orders ALWAYS go to existing batches until 3500kg';
    RAISE NOTICE '   • New batches only created when existing one is FULL';
    RAISE NOTICE '   • One batch per barangay until capacity reached';
    RAISE NOTICE '========================================';
END $$; 