-- Add email trigger for delivery status changes
-- This will automatically send emails when orders go out for delivery

-- Function to send delivery status emails
...
BEGIN
    ...
    IF (NEW.delivery_status = 'out_for_delivery' OR NEW.delivery_status = 'delivering') 
       AND (OLD.delivery_status != 'out_for_delivery' AND OLD.delivery_status != 'delivering') THEN
        ...
        -- Call the quick-processor function to send email
        PERFORM net.http_post(
            url := 'https://vpwskrytguoiybqrpebp.supabase.co/functions/v1/quick-processor',
        ...
        RAISE NOTICE 'Delivery email sent for order % to %', NEW.id, customer_email;
    END IF;
    
    RETURN NEW;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error sending delivery email for order %: %', NEW.id, SQLERRM;
        RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create trigger for delivery status changes
DROP TRIGGER IF EXISTS delivery_email_trigger ON orders;
CREATE TRIGGER delivery_email_trigger
