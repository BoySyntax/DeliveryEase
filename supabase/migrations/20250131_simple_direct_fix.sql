-- SIMPLE DIRECT FIX: This will work for every approved order
-- No complex functions, just direct batching based on address
...
-- Step 2: Create a simple function to get barangay from address
CREATE OR REPLACE FUNCTION get_barangay_simple(delivery_address jsonb)
RETURNS text AS $$
DECLARE
    barangay text;
    full_address text;
BEGIN
    ...
    -- Look for specific barangays in the address
...
-- Step 3: Create a simple batching function
...
BEGIN
    ...
    IF NEW.approval_status = 'approved' AND OLD.approval_status != 'approved' THEN
        ...
        order_barangay := get_barangay_simple(NEW.delivery_address);
...
-- Step 6: Create function to manually batch all approved orders
...
BEGIN
    -- Get all approved orders without batch_id
    FOR order_record IN 
    ...
    LOOP
        -- Get barangay from address
        order_barangay := get_barangay_simple(order_record.delivery_address);
...
BEGIN
    ...
    LOOP
        ...
        IF order_record.delivery_address IS NOT NULL THEN
            detected_barangay := get_barangay_simple(order_record.delivery_address);
...
-- Step 9: Create a simple test function
CREATE OR REPLACE FUNCTION test_simple_batching()
RETURNS TABLE(
    test_case text,
    input text,
    result text,
...
BEGIN
    ...
    SELECT 
        ...
        get_barangay_simple('{"barangay": "Patag, Misamis Oriental"}'::jsonb),
        (get_barangay_simple('{"barangay": "Patag, Misamis Oriental"}'::jsonb) = 'Patag, Misamis Oriental');
    ...
    SELECT 
        ...
        get_barangay_simple('{"address": "FJQ9+J7X, Cagayan De Oro City, • Misamis Oriental, Philippines, Patag, Misamis Oriental"}'::jsonb),
        (get_barangay_simple('{"address": "FJQ9+J7X, Cagayan De Oro City, • Misamis Oriental, Philippines, Patag, Misamis Oriental"}'::jsonb) = 'Patag, Misamis Oriental');
    ...
    SELECT 
        ...
        get_barangay_simple('{"address": "Zone 2 lower tabok, Cagayan De Oro City, Misamis Oriental, Philippines, Bulua, Misamis Oriental"}'::jsonb),
        (get_barangay_simple('{"address": "Zone 2 lower tabok, Cagayan De Oro City, Misamis Oriental, Philippines, Bulua, Misamis Oriental"}'::jsonb) = 'Bulua, Misamis Oriental');
END;
$$ LANGUAGE plpgsql;

-- Step 10: Run the test
SELECT * FROM test_simple_batching();
...
SELECT o.id, p.name, get_barangay_simple(o.delivery_address) as barangay, o.total_weight, o.created_at
FROM orders o
